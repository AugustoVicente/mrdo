	...
	while(status == '1'){  
		...
		switch(keypress){
			...
			case 224:
				...
				if(keypress == 75 && limiteMapa(Do.x-1, Do.y) == 1)	Do.x--;//esquerda
				if(keypress == 77 && limiteMapa(Do.x+1, Do.y) == 1) Do.x++;//direita
				if(keypress == 80 && limiteMapa(Do.x, Do.y+1) == 1) Do.y++;//baixo
				if(keypress == 72 && limiteMapa(Do.x, Do.y-1) == 1) Do.y--;//cima	
			...
			for (i = 0 ; i < contFrutas; i++)		
    			if ((totalFrutas[i]) == 1){
    				contador++;
    				score = contador*100;//Cem pontos cada fruta comida
				}  
