...
char movimentoMrDo (mrDo Do, int contFrutas, int *xFrutas, int *yFrutas, char **mapa, int *xyNinho) {
	...
	int totalFrutas[contFrutas], score = 0, contador = 0, fantasmaScore = 0;
	Tiro tiro;
	fantasma fant;
	fant.x = xyNinho[0];
	fant.y = xyNinho[1] + 1;//posicao inicial do fantasma 1 pixel abaixo do ninho
	fant.status = 1; //vivo
	fant.ultima = 3; // inicia para baixo
	
	for (i = 0; i < contFrutas; i++) totalFrutas[i] = 0; //inicia o vetor de frutas comidas em zero.
	
	ShowConsoleCursor(0);
	
	gotoxy(fant.x, fant.y);
	SetColor(15);
	printf ("%c", 153); // desenha o Fantasma
	...
	while(status == '1'){  
		...
		switch(keypress){
			...
			case 224:
				...
				//apaga o mrDo na posicao anterior com excecao do ninho
				if (Do.x == xyNinho[0] && Do.y ==xyNinho[1]){
						SetColor (8);
						gotoxy(Do.x, Do.y);
						printf ("%c", 206);
						SetColor(14);
			...
			if (tiro.status == 1){
				...
				if ( a > 0){
					if (tiro.x == xyNinho[0] && tiro.y == xyNinho[1]){
						SetColor (8);
						gotoxy(tiro.x, tiro.y);
						printf ("%c", 206);//nao apaga o ninho quando o o tiro passar por ele
						SetColor(14);
					}
					else{
						gotoxy(tiro.x,tiro.y);
				...
				if ((abs(tiro.x - fant.x) < 2) && (abs(tiro.y - fant.y) < 2)){// Se o tiro acertar o fantasma mata ele e soma 10 pontos
					gotoxy(fant.x, fant.y);
					printf (" ");
					fant.status = 0;
					fantasmaScore = 10; 
			...
			if (fant.status == 1){
				...
				SetColor(15);
					...
					if (fant.x == xyNinho[0] && fant.y == xyNinho[1]){
						SetColor (8);
						gotoxy(fant.x, fant.y);
						printf ("%c", 206);//nao apaga o ninho quando o fantasma passar por ele
						SetColor(15);
					} else{							
						gotoxy(fant.x,fant.y);
						printf (" ");
				...
				if((fant.ultima == 0) && limiteMapa(fant.x-1, fant.y) == 1){
					
					if (mapa[fant.y][fant.x-1] == 'p')
						while ( fant.ultima == 0) fant.ultima = rand() % 4;
					
					else {
						
						fant.x--;//esquerda
						gotoxy(fant.x, fant.y);
						printf ("%c", 153);// imprime fantasma
					}
				}
				
				if((fant.ultima == 1) && limiteMapa(fant.x+1, fant.y) == 1){
					
					if (mapa[fant.y][fant.x+1] == 'p')
						while (fant.ultima == 1) fant.ultima = rand() % 4;
				...
				if((fant.ultima == 3) && limiteMapa(fant.x, fant.y+1) == 1){
					if (mapa[fant.y+1][fant.x] == 'p')
						while (fant.ultima == 3) fant.ultima = rand() % 4; 
				...
				if((fant.ultima == 2) && limiteMapa(fant.x, fant.y-1) == 1){
					if (mapa[fant.y-1][fant.x] == 'p')
						while (fant.ultima == 2) fant.ultima = rand() % 4;
			...
			score = 50*contador + fantasmaScore;
			
		if (contador == contFrutas) status = '0';//Comeu todas as frutas e passa para a proxima fase 
		
		if ((Do.x == fant.x) && (Do.y == fant.y)) status = '2'; // Se o fantasma acertar o Mr Do ele morre
