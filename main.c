...
void main(){
 ...
 int *xFrutas, *yFrutas, xyNinho[2], i, j; // declara os vetores das posicoes das frutas
	...
	while (opcao == '0'){ // Forca o inicio para a tela de menu inicial
		...
		opcao = menuInicial();// escolha opcao de 1 a 4 no menu 
			...
			while (opcao == '1'){ // Opcao 1 - "Novo Jogo" - Inicia o jogo
				...
				system("cls");
				carregarFase(url, xFrutas, yFrutas, mapa, xyNinho);// Le arquivo da fase e desenha ela na tela;
				/*for (i = 0; i < LINHA; i++){
					for (j = 0; j < COLUNA; j++)
						printf ("%c", mapa[i][j]);
					printf ("\n");					
...
//Movimento do Mr. Do (dentro da fase)
				...
				Do.y = 22;
				Do.ultima = 72;//CIMA
				opcao = movimentoMrDo(Do, contFrutas, xFrutas, yFrutas, mapa, xyNinho);
